...
def apply_sobel_filter(image, dx, dy):
    return cv2.Sobel(image, cv2.CV_64F, dx, dy, ksize=3)

def calculate_gradient_magnitude(image, threshold):
    dx = apply_sobel_filter(image, 1, 0)
    dy = apply_sobel_filter(image, 0, 1)
    gradient_squared = dx**2 + dy**2
    gradient_magnitude = np.sqrt(gradient_squared)
    return np.where(gradient_magnitude < threshold, 0, 1).astype(np.uint8) * 255

# Carregar a imagem de exemplo
image = cv2.imread('C:/Users/Samsung/tarefa_1.2/imagem2.jpg', cv2.IMREAD_GRAYSCALE)

# Suavizar a imagem aplicando um filtro Gaussiano
smoothed_image = cv2.GaussianBlur(image, (5, 5), 0)

# Calcular as derivadas parciais nas direções x e y
dx = apply_sobel_filter(smoothed_image, 1, 0)
dy = apply_sobel_filter(smoothed_image, 0, 1)
...
gradient_squared_sum = dx_squared + dy_squared

# Tirar a raiz quadrada de cada elemento da matriz
gradient_magnitude = np.sqrt(gradient_squared_sum)

# Escolher um valor de threshold
threshold = 30.0 # quanto menor o thershold mais pixels brancos

# Gerar a matriz final (D)
final_image = calculate_gradient_magnitude(smoothed_image, threshold)
